...
\label{mmsec}
\subsection{Model Description} 
The second question we wanted to address
- comparison with pe'er paper
\subsection{Implementation} 
\subsection{Experiments}  
- 500 sampling iterations per run
\begin{table}[h]
    \centering
    \label{mmtable}
    \begin{tabular}{@{}ll@{}}
        ...
        100                & 20                                                        \\ \bottomrule
    \end{tabular}
    \caption{The number of clusters defined in the model appears to be more than the actual number of unique clusters assigned to the data points after posterior estimation. The estimated true number of categories is slightly above 10.}
\end{table}
...
For our Gibb's sampler, we had a fixed burn-in of number of samples (200), a fixed number of sampling iterations after that (500). For time reasons, we didn't extensively explore varying these values. Figure~\ref{fig:llmm} describes the log-likelihood evaluation of the estimated posteriors.

\begin{figure*}
    \centering
    \includegraphics[width=1\textwidth]{figs/llmm}
